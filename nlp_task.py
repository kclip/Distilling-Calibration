...
def parseArgs():
    ...
    parser.add_argument('--quantize', type=str, default=torch.qint8, help='weight quantized')
...
def D_alpha(p, q, alpha):
    ...
    divergence = np.sum(np.multiply(new_p, new_q))
...
def main(args):
    ...
    for i in range(large_calibration_probs.shape[0]):
        ...
        calibration_score['alpha'].append(D_alpha(large_calibration_probs[i], small_calibration_probs[i], alpha=args.alpha_div))
        # calibration_score['tv'].append(0.5 * np.sum(np.abs(large_calibration_probs[i] - small_calibration_probs[i])))
        # calibration_score['inner'].append(1 - np.inner(large_calibration_probs[i], small_calibration_probs[i]))
        # calibration_score['ws'].append(wasserstein_distance([0, 1, 2], [0, 1, 2], large_calibration_probs[i], small_calibration_probs[i]))
    ...
    for name in distance_function:
        ...
        print(f'NC function: {name}, {name}\'s quantile: {calibration_score_quantile[name]}')
                ...
                if name == 'kl':
                    ...
                    for j in range(simplex.shape[0]):
                        if D_alpha(simplex[j], small_test_probs[k][i], alpha=args.alpha_div) < calibration_score_quantile['alpha']:
                            idx_kl.append(j)
                    set_size_test['alpha'].append(len(idx_kl))
